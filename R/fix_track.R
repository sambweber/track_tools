...
fix_track = function (x, dt = 'datetime', drop.duplicates = F, eps = 10){
  ...
  x %>%
    ...
    mutate(!!dt := adjust_duplicate_times(!!dt,drop = drop.duplicates, eps=eps)) %>% # adjust duplicated time stamps
